      ...
      <el-tab-pane label="系统图片" name="system">
        <el-table :data="systemImageList" stripe style="width: 100%; margin-top: 20px;">
          ...
          <el-table-column label="操作" width="180">
            <template #default="scope">
              <el-button type="text" size="small" @click="handleSetBackDefault(scope.row)" v-if="scope.row.pictureId!=defaultSet.setBack">设为默认聊天背景</el-button>
              <el-button type="text" size="small" @click="handleDelete(scope.row, 'system')" v-if="scope.row.pictureId!=defaultSet.setBack">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
...
// 设为默认操作
const handleSetBackDefault = async (row) => {
