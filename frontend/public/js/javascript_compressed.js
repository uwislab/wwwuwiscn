...
Blockly.JavaScript.lists_repeat = function (a) {
    var b = Blockly.JavaScript.provideFunction_("listsRepeat",
            ...
            "(value, n) {", "  var array = [];", "  for (var i = 0; i < n; i++) {", "    array[i] = value;", "  }", "  return array;", "}"]),
...
Blockly.JavaScript.lists_getSublist = function (a) {
    ...
    if ("FIRST" == c && "LAST" == d)
        ...
        var g = {FIRST: "First", LAST: "Last", FROM_START: "FromStart", FROM_END: "FromEnd"};
        ...
        e = f + "(" + b + ("FROM_END" == c || "FROM_START" == c ? ", " + e : "") + ("FROM_END" == d || "FROM_START" == d ? ", " + a : "") + ")"
    }
...
Blockly.JavaScript.move_forward = function (block) {
    // number_level range 1 to 3
    var number_level = block.getFieldValue('level');
    var number_distance = block.getFieldValue('distance');

    number_level = limitNumber(number_level, 1, 8);
    number_distance = limitNumber(number_distance, 0, Number.POSITIVE_INFINITY);

    var code = 'forward(' + number_level + ',' + number_distance + ');\n';
    console.log(code);
    return code;
};
...
Blockly.JavaScript.move_back = function (block) {
    ...
    var code = 'back(' + number_level + ',' + number_distance + ');\n';
    return code;
};
...
Blockly.JavaScript.turn_left = function (block) {
    ...
    angle_angle = limitNumber(angle_angle, 0, 360);

    var code = 'turn_left(' + angle_angle + ');\n';
    return code;
};
...
Blockly.JavaScript.turn_right = function (block) {
    ...
    // angle_angle = limitNumber(angle_angle, Number.MIN_VALUE, Number.MIN_VALUE);
    angle_angle = limitNumber(angle_angle, 0, 360);

    var code = 'turn_right(' + angle_angle + ');\n';
    return code;
};
...
Blockly.JavaScript.start_super = function (block) {
    var code = 'tracker_start();\n';
    return code;
};
Blockly.JavaScript.stop_super = function (block) {
    var code = 'tracker_close();\n';
    return code;
};
Blockly.JavaScript.servo_run = function (block) {
    var angle_angle = block.getFieldValue('angle');

    // angle_angle = limitNumber(angle_angle, Number.MIN_VALUE, Number.MIN_VALUE);
    angle_angle = limitNumber(angle_angle, 0, 360);

    var code = 'servo_run(' + angle_angle + ');\n';
    return code;
};
Blockly.JavaScript.inf_emission = function (block) {
    var bound = block.getFieldValue('bound');

    bound = limitNumber(bound, 1, 1000);

    var code = 'inf_emission(' + bound + ');\n';
    return code;
};

/*****************外观控制********************/
Blockly.JavaScript.light_color_change = function (block) {
    var index = block.getFieldValue('index');

    var ccolor = block.getFieldValue('color');


    var code = 'colorful_led(' + index + ',' + ccolor + ');\n';
    return code;
};

// Blockly.JavaScript.light_off = function(block) {
//     var code = 'uBlocks.light_off();\n';
//     return code;
// };

Blockly.JavaScript.play_sounds = function (block) {
    var role = block.getFieldValue('sounds');
    var contain = block.getFieldValue('sentences');

    var code = 'play(\'' + role + '\',\'' + contain + '\');\n';
    return code;
};
...
Blockly.JavaScript.speak = function (block) {
    ...
    var contain = block.getFieldValue('sentences');
    if (role == 'man') {
        role = role.replace('man', 'm51');
    }
    if (role == 'women') {
        role = role.replace('women', 'm3');
    }
    if (role == 'duck') {
        role = role.replace('duck', 'm54');
    }
    role = '[' + role + ']' + contain;
    var code = 'gpp_say(1,\'' + role + '\');\n';
    return code;
};
Blockly.JavaScript.beeper = function (block) {
    var bound = block.getFieldValue('bound');
    var code = 'beep(' + bound + ');\n';
    return code;
};
Blockly.JavaScript.poem = function (block) {
    var role = 'm51';
    var contain = '桃花溪 作者张旭 隐隐飞桥隔野烟，石矶西畔问渔船。 桃花尽日随流水，洞在清溪何处边.';
    role = '[' + role + ']' + contain;
    var code = 'gpp_say(1,\'' + role + '\');\n';
    return code;
};
Blockly.JavaScript.wait = function (block) {
    ...
    //number_second = number_second * 20;
    var code = 'wait(' + number_second + ');\n';
    return code;
};
/**************** 传感器 ***********/
Blockly.JavaScript.sensor_light = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_ltSensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = lightsensor();\n';
    return code;
};
Blockly.JavaScript.sensor_distance = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_ifSensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = distsensor();\n';
    return code;
};
Blockly.JavaScript.sensor_mic = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_m1Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = mic1sensor();\n';
    return code;
};
Blockly.JavaScript.sensor_irm = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_irmSensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = irmsensor();\n';
    return code;
};
Blockly.JavaScript.sensor_tra1 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA1Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker(1);\n';
    return code;
};
Blockly.JavaScript.sensor_tra2 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA2Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker(2);\n';
    return code;
};
Blockly.JavaScript.sensor_tra3 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA3Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker(3);\n';
    return code;
};
Blockly.JavaScript.sensor_tra4 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA4Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker(4);\n';
    return code;
};
Blockly.JavaScript.sensor_tra5 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA4Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker_plus(1);\n';
    return code;
};
Blockly.JavaScript.sensor_tra6 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA4Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker_plus(2);\n';
    return code;
};
Blockly.JavaScript.sensor_tra7 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA4Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker_plus(3);\n';
    return code;
};
Blockly.JavaScript.sensor_tra8 = function (block) {
    // var code = 'uBlocks.sensor(\'sensor_value_tA4Sensor\');\n';
    // return code;
    var var_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'),
        Blockly.Variables.NAME_TYPE);
    var code = var_name + ' = tracker_plus(4);\n';
    return code;
};

Blockly.JavaScript.Get_Ps2Value = function () {
    var code = 'sensor_value_ps2value;\n';
    return code;
};

Blockly.JavaScript.control_delay = function (block) {
    var number_time = block.getFieldValue('time');
    number_time = number_time * 20;
    var code = 'wait(' + number_time + ');\n';
    return code;
};

Blockly.JavaScript.control_loops_times = function (block) {
    var number_time = block.getFieldValue('time');
    var statements_next = Blockly.JavaScript.statementToCode(block, 'next');

    var code = 'for (var i = 0; i < ' + number_time + '; i++) {\n';
    code += statements_next + '}\n';
    return code;
};

Blockly.JavaScript.control_loops_forever = function (block) {
    var statements_next = Blockly.JavaScript.statementToCode(block, 'next');
...
Blockly.JavaScript.Set_CScript_Mode = function (block) {
    var mode = block.getFieldValue('mode');
    var code = 'Set_CScript_Mode(' + mode + ');\n';
    return code;
};
...
Blockly.JavaScript.variable_set = function (block) {
    ...
    var code = variable_name + ' = ' + text_value + ';\n';
    console.log(code);
    return code;
};

Blockly.JavaScript.variable_plus = function (block) {
    var variable_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'), Blockly.Variables.NAME_TYPE);
    var text_value = block.getFieldValue('value');
...
Blockly.JavaScript.variable_if = function (block) {
    var variable_name = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('name'), Blockly.Variables.NAME_TYPE);
    var dropdown_op = block.getFieldValue('op');
    var text_value = block.getFieldValue('value');
    var statements_next = Blockly.JavaScript.statementToCode(block, 'next');

    var code = 'if (' + variable_name + ' ' + dropdown_op + ' ' + text_value + ') {\n';
    code += statements_next + '}\n';
    return code;
};

Blockly.JavaScript.servo_open = function () {
    alert('clicked');
    var code = 'servo_open();\n';
    return code;
};

Blockly.JavaScript.servo_close = function () {
    var code = 'servo_close();\n';
    return code;
};
/** uBlocks  function **/
...
uBlocks.server = {
    host: 'http://www.uwis.dev',//http://www.uwis.cn
    port: 9090
};

uBlocks.start = function () {
    var url = uBlocks.wrapper('start();');
    // uBlocks.send(url);
};

uBlocks.forward = function (level, distance, async) {
    async = async !== undefined;
    var str = 'forward(' + level + ',' + distance + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};

uBlocks.back = function (level, distance, async) {
    async = async !== undefined;
    var str = 'back(' + level + ',' + distance + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};

uBlocks.turn_left = function (angle, async) {
    async = async !== undefined;
    var str = 'turn_left(' + angle + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};

uBlocks.turn_right = function (angle, async) {
    async = async !== undefined;
    var str = 'turn_right(' + angle + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};

uBlocks.stop = function (async) {
    async = async !== undefined;
    var url = uBlocks.wrapper('stop();');
    uBlocks.send(url, async);
};

uBlocks.servo_run = function (angle, async) {
    async = async !== undefined;
    var str = 'servo(1,' + angle + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};
uBlocks.inf_emission = function (bound, async) {
    async = async !== undefined;
    var str = 'inf_emission(' + bound + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};
...
/********* 外观控制 ************/
uBlocks.light_change = function (index, color, async) {
    async = async !== undefined;
    var str = 'colorful_led(' + index + ',' + color + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};

// uBlocks.light_off = function (async) {
//     async = async !== undefined;
//     var url = uBlocks.wrapper('light_off();');
//     uBlocks.send(url, async);
// };

uBlocks.play = function (role, sentences, async) {
    async = async !== undefined;
    var str = '';
    str = (role === "man") ? ('gpp_say(0,\"[m51]' + sentences + '\")') : (
        (role === "woman") ? ('gpp_say(0,\"[m3]' + sentences + '\")') :
            (('gpp_say(0,\"[m54]' + sentences + '\")')));

    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};

uBlocks.gpp_say = function (role, sentences, async) {
    async = async !== undefined;
    var str = '';
    str = (role === "man") ? ('gpp_say(1,\"[m51]' + sentences + '\")') : (
        (role === "woman") ? ('gpp_say(1,\"[m3]' + sentences + '\")') :
            (('gpp_say(1,\"[m54]' + sentences + '\")')));

    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};
uBlocks.beep = function (bound, async) {
    async = async !== undefined;
    var str = 'beep(' + bound + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
...
/************** 传感器 *******************/
uBlocks.sensor = function (sensor, async) {
    async = async !== undefined;
    var url = uBlocks.wrapper(sensor);
    uBlocks.send(url, async);
}

uBlocks.wait = function (seconds, async) {
    async = async !== undefined;
    var str = 'wait(' + seconds + ');';
    var url = uBlocks.wrapper(str);
    uBlocks.send(url, async);
};
...
uBlocks.auth = function () {
    return '&id=' + uBlocks.car.id + '&password=' + uBlocks.car.password;
};

uBlocks.wrapper = function (direction) {
    var url = uBlocks.server.host + ':' + uBlocks.server.port + '/?cmd=';

    url += direction + uBlocks.auth();

    return url;
};

uBlocks.send = function (url, async) {
    var response;
...
uBlocks.reportURL = function (sensor, async) {
    var url = uBlocks.wrapper(sensor);
    return uBlocks.send(url);
};

/********utils***********/

function limitNumber(a, low, high) {
    var res = low;
