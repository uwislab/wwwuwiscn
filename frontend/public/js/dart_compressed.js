...
Blockly.Dart.init = function (a) {
    ...
    b.length && (Blockly.Dart.definitions_.variables =b.join(", ")+ ';\n')
};
Blockly.Dart.finish = function (a) {

    a && (a = Blockly.Dart.prefixLines(a, Blockly.Dart.INDENT));

    var variables = Blockly.Dart.definitions_.variables;
    a = '  int ' + variables + a;

    a = "#include <stdio.h>\nint main() {\n" + a + "}";
    if (a.indexOf("rand() % ") != -1) {
        a = a.slice(0,19)+'#include <stdlib.h>\n'+ a.slice(19)
    }
    var b = [], c = [], d;
...
Blockly.Dart.scrub_ = function (a, b) {
    ...
    if (!a.outputConnection || !a.outputConnection.targetConnection) {
        ...
        (d = Blockly.utils.wrap(d, Blockly.Dart.COMMENT_WRAP - 3)) && (c = a.getProcedureDef ? c + Blockly.Dart.prefixLines(d + "\n", "/// ") : c + Blockly.Dart.prefixLines(d + "\n", "// "));
        for (var e = 0; e < a.inputList.length; e++) a.inputList[e].type == Blockly.INPUT_VALUE && (d = a.inputList[e].connection.targetBlock()) && (d = Blockly.Dart.allNestedComments(d)) && (c += Blockly.Dart.prefixLines(d, "// "))
    }
    e = a.nextConnection &&
        a.nextConnection.targetBlock();
    e = Blockly.Dart.blockToCode(e);
    return c + b + e
};
...
Blockly.Dart.colour_picker = function (a) {
    return ["'" + a.getFieldValue("COLOUR") + "'", Blockly.Dart.ORDER_ATOMIC]
};
...
Blockly.Dart.colour_rgb = function (a) {
    ...
    Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
    return [Blockly.Dart.provideFunction_("colour_rgb", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(num r, num g, num b) {", "  num rn = (Math.max(Math.min(r, 100), 0) * 2.55).round();", "  String rs = rn.toInt().toRadixString(16);",
...
Blockly.Dart.colour_blend = function (a) {
    ...
    Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
    return [Blockly.Dart.provideFunction_("colour_blend", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String c1, String c2, num ratio) {", "  ratio = Math.max(Math.min(ratio, 1), 0);",
        ...
        "  num bn = (b1 * (1 - ratio) + b2 * ratio).round();", "  String bs = bn.toInt().toRadixString(16);", "  rs = '0$rs';", "  rs = rs.substring(rs.length - 2);", "  gs = '0$gs';", "  gs = gs.substring(gs.length - 2);", "  bs = '0$bs';", "  bs = bs.substring(bs.length - 2);", "  return '#$rs$gs$bs';", "}"]) + "(" + b + ", " + c + ", " + a + ")", Blockly.Dart.ORDER_UNARY_POSTFIX]
};
Blockly.Dart.lists = {};
Blockly.Dart.addReservedWords("Math");
...
Blockly.Dart.lists_getIndex = function (a) {
    ...
    if (("RANDOM" != d || "REMOVE" != c) && "FROM_END" != d || e.match(/^\w+$/)) switch (d) {
        ...
        case "FROM_START":
            ...
            ".removeAt(" + d + ")", Blockly.Dart.ORDER_UNARY_POSTFIX];
        ...
        case "RANDOM":
            ...
            if ("GET_REMOVE" == c) return c = Blockly.Dart.provideFunction_("lists_remove_random_item",
                ["dynamic " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List my_list) {", "  int x = new Math.Random().nextInt(my_list.length);", "  return my_list.removeAt(x);", "}"]), [c + "(" + e + ")", Blockly.Dart.ORDER_UNARY_POSTFIX]
    } else {
        ...
        if ("GET_REMOVE" == c) return d = Blockly.Dart.getAdjusted(a, "AT", 1), c = Blockly.Dart.provideFunction_("lists_remove_from_end",
            ["dynamic " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List my_list, num x) {", "  x = my_list.length - x;", "  return my_list.removeAt(x);", "}"]), [c + "(" + e + ", " + d + ")", Blockly.Dart.ORDER_UNARY_POSTFIX]
    }
...
Blockly.Dart.lists_getSublist = function (a) {
    ...
    if (b.match(/^\w+$/) || "FROM_END" != c && "FROM_START" == d) {
        ...
        switch (d) {
            case "FROM_START":
                var f =
                    Blockly.Dart.getAdjusted(a, "AT2", 1);
                break;
            case "FROM_END":
    ...
    } else e = Blockly.Dart.getAdjusted(a, "AT1"), f = Blockly.Dart.getAdjusted(a, "AT2"), a = Blockly.Dart.provideFunction_("lists_get_sublist", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(list, where1, at1, where2, at2) {",
...
Blockly.Dart.lists_sort = function (a) {
    ...
    return [Blockly.Dart.provideFunction_("lists_sort", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(list, type, direction) {", "  var compareFuncs = {", '    "NUMERIC": (a, b) => direction * a.compareTo(b),', '    "TEXT": (a, b) => direction * a.toString().compareTo(b.toString()),', '    "IGNORE_CASE": ', "       (a, b) => direction * ",
...
Blockly.Dart.controls_if = function (a) {
    ...
    a.getInput("ELSE") && (e = Blockly.Dart.statementToCode(a, "ELSE"), c += " else {\n" + e + "}");
    return c + "\n"
};
...
Blockly.Dart.controls_repeat_ext = function (a) {
    ...
    c = Blockly.Dart.addLoopTrap(c, a.id);
    a = "";
    var d = Blockly.Dart.variableDB_.getDistinctName("count", Blockly.Variables.NAME_TYPE), e = b;
...
Blockly.Dart.controls_whileUntil = function (a) {
    ...
    d = Blockly.Dart.addLoopTrap(d, a.id);
    b && (c = "!" + c);
    return "while (" + c + ") {\n" + d + "}\n"
};
Blockly.Dart.controls_for = function (a) {
    ...
    f = Blockly.Dart.addLoopTrap(f, a.id);
...
Blockly.Dart.math_number_property = function (a) {
    ...
    if ("PRIME" == c) return Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;", [Blockly.Dart.provideFunction_("math_isPrime", ["bool " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(n) {", "  // https://en.wikipedia.org/wiki/Primality_test#Naive_methods", "  if (n == 2 || n == 3) {", "    return true;",
        "  }", "  // False if n is null, negative, is 1, or not whole.", "  // And false if n is divisible by 2 or 3.", "  if (n == null || n <= 1 || n % 1 != 0 || n % 2 == 0 || n % 3 == 0) {", "    return false;", "  }", "  // Check all the numbers of form 6k +/- 1, up to sqrt(n).", "  for (var x = 6; x <= Math.sqrt(n) + 1; x += 6) {", "    if (n % (x - 1) == 0 || n % (x + 1) == 0) {", "      return false;", "    }", "  }", "  return true;", "}"]) + "(" + b + ")", Blockly.Dart.ORDER_UNARY_POSTFIX];
    switch (c) {
        case "EVEN":
            var d =
                b + " % 2 == 0";
...
Blockly.Dart.math_on_list = function (a) {
    ...
    switch (b) {
        case "SUM":
            b = Blockly.Dart.provideFunction_("math_sum", ["num " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List myList) {", "  num sumVal = 0;", "  myList.forEach((num entry) {sumVal += entry;});", "  return sumVal;", "}"]);
            b = b + "(" + a + ")";
            break;
        case "MIN":
            Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
            b = Blockly.Dart.provideFunction_("math_min",
                ["num " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List myList) {", "  if (myList.isEmpty) return null;", "  num minVal = myList[0];", "  myList.forEach((num entry) {minVal = Math.min(minVal, entry);});", "  return minVal;", "}"]);
            b = b + "(" + a + ")";
            break;
        case "MAX":
            Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
            b = Blockly.Dart.provideFunction_("math_max", ["num " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List myList) {", "  if (myList.isEmpty) return null;", "  num maxVal = myList[0];",
        ...
        case "MODE":
            Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
            b = Blockly.Dart.provideFunction_("math_modes", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List values) {", "  List modes = [];", "  List counts = [];", "  int maxCount = 0;", "  for (int i = 0; i < values.length; i++) {", "    var value = values[i];",
...
Blockly.Dart.math_random_int = function (a) {
    ...
    var result;
    if (b == "0") {
        result =  'rand() % ' + (Number(a) + 1);
    } else {
        result =  'rand() % ' + (Number(a) - Number(b) + 1) + '+' + b;
    }
    return [result, Blockly.Dart.ORDER_UNARY_POSTFIX]
};
...
Blockly.Dart.procedures_defreturn = function (a) {
    var b = Blockly.Dart.variableDB_.getName(a.getFieldValue("NAME"), Blockly.Procedures.NAME_TYPE),
        c = Blockly.Dart.statementToCode(a, "STACK");
    if (Blockly.Dart.STATEMENT_PREFIX) {
        var d = a.id.replace(/\$/g, "$$$$");
        c = Blockly.Dart.prefixLines(Blockly.Dart.STATEMENT_PREFIX.replace(/%1/g, "'" + d + "'"), Blockly.Dart.INDENT) + c
    }
...
Blockly.Dart.text_charAt = function (a) {
    ...
    switch (b) {
        ...
        case "LAST":
        case "FROM_END":
            return a = Blockly.Dart.getAdjusted(a, "AT", 1), b = Blockly.Dart.provideFunction_("text_get_from_end", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ +
            "(String text, num x) {", "  return text[text.length - x];", "}"]), [b + "(" + c + ", " + a + ")", Blockly.Dart.ORDER_UNARY_POSTFIX];
        case "RANDOM":
...
Blockly.Dart.text_changeCase = function (a) {
    ...
    return [b ? a + b : Blockly.Dart.provideFunction_("text_toTitleCase", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String str) {", "  RegExp exp = new RegExp(r'\\b');", "  List<String> list = str.split(exp);", "  final title = new StringBuffer();", "  for (String part in list) {",
...
Blockly.Dart.text_print = function (a) {
    return "printf(" + (Blockly.Dart.valueToCode(a, "TEXT", Blockly.Dart.ORDER_NONE) || "\"\"") + ");\n"
};
...
Blockly.Dart.text_count = function (a) {
    ...
    return [Blockly.Dart.provideFunction_("text_count", ["int " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String haystack, String needle) {", "  if (needle.length == 0) {", "    return haystack.length + 1;", "  }", "  int index = 0;", "  int count = 0;", "  while (index != -1) {", "    index = haystack.indexOf(needle, index);", "    if (index != -1) {",
        "      count++;", "     index += needle.length;", "    }", "  }", "  return count;", "}"]) + "(" + b + ", " + a + ")", Blockly.Dart.ORDER_UNARY_POSTFIX]
};
